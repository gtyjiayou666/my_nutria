// <input-method-desktop> web component

class InputMethodDesktop extends HTMLElement {
  constructor() {
    super();
    this.flag = true;
    this.loaded = false;
  ...
  connectedCallback() {
    this.innerHTML = `
    <link rel="stylesheet" href="components/input_method_desktop.css">
    <web-view
      remote="true"
  ...
  init() {
    if (!this.loaded) {
      let keyboardUrl = `http://keyboard-desktop.localhost:${config.port}/index.html`;
...
customElements.define("input-method-desktop", InputMethodDesktop);
